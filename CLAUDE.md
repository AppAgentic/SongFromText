...
- **Analytics**: Firebase Analytics for product funnel events + Meta Pixel/Conversions API for paid-social attribution
...
- **Firebase Analytics** — product/funnel analytics for `create_viewed`, `messages_ready`, `sound_selected`, `email_submitted`, checkout, and generation events. Requires Firebase Analytics/GA4 enabled and `NEXT_PUBLIC_FIREBASE_MEASUREMENT_ID` set. It no-ops safely until the measurement id exists.
- **Meta Ads attribution** — use browser Pixel + server CAPI together. Generate unique event ids per conversion action; send the same id as Pixel `eventID` and CAPI `event_id` for dedupe. Capture `_fbp`, `_fbc`, `fbclid`, UTM params, landing page, referrer, client IP, and user agent before Whop redirect. Send `Lead` on valid email capture, `InitiateCheckout` before redirect, and `Purchase` from the Whop webhook using stored project attribution. Do not depend on Whop-hosted checkout alone for attribution.
...
- Add `NEXT_PUBLIC_FIREBASE_MEASUREMENT_ID` after Firebase Analytics/GA4 is enabled for the web app.
